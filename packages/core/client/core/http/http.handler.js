...
/**
 * Implementation of the {@codelink Core.Interface.Http} interface with
 * internal caching of completed and ongoing HTTP requests and cookie storage.
 *
 * @class Handler
 * @implements Core.Interface.Http
 * @namespace Core.Http
 * @module Core
 * @submodule Core.Http
 *
...
class Handler extends ns.Core.Interface.Http {
	...
	_proxyRejected(errorParams) {
		...
		if (errorParams.options.repeatRequest > 0) {
			...
			var errorMessage = `${errorName}: Core.Http.Handler:_proxyRejected`
			var error = ns.oc.create('$Error', errorMessage, errorParams)
