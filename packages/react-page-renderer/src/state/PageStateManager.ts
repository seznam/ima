import PageStateEvents from './PageStateEvents';
...
/**
 * The implementation of the {@linkcode PageStateManagerInterface} interface.
 */
export default class PageStateManager implements PageStateManagerInterface {
  ...
  setState(patchState: object) {
    ...
    if (this._dispatcher) {
      this._dispatcher.fire(
        PageStateEvents.BEFORE_CHANGE_STATE,
  ...
  _callOnChangeCallback(newState: object) {
    ...
    if (this._dispatcher) {
      this._dispatcher.fire(PageStateEvents.AFTER_CHANGE_STATE, { newState }, true);
    }
  }
}
