...
function getDevLanguageAssets(
  ...
  return Object.fromEntries(
    Object.entries(config.build?.rolldownOptions?.input ?? {}).flatMap(
      ([inputKey, inputValue]) => {
...
function getPolyfillAssets(
  ...
  if (input && typeof input === 'object' && POLYFILL_ENTRY_KEY in input) {
    const assetFilePath = path.join(
      outputDir,
      path.basename(input[POLYFILL_ENTRY_KEY])
    );
    polyfillAssets[assetFilePath] = {
      fileName: input[POLYFILL_ENTRY_KEY],
      name: assetFilePath,
    };
  }
 ...
 */
export async function createManifestFileForDev(
  imaConfig: ImaConfig,
  config: ViteConfigWithEnvironments
) {
  const serverAssets = {
 ...
 */
export async function createManifestFileFromOutput(
  results: ImaBuildOutput[],
  imaConfig: ImaConfig
) {
  const manifest: Manifest = {
    assets: {},
 ...
 */
function extendManifestFromOutput(
  manifest: Manifest,
  env: string,
