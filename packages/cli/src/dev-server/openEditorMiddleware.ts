...
/**
 * Try to figure out the best editor and return it.
 */
function guessEditor(): string | null {
  ...
  try {
    if (process.platform === 'darwin') {
      ...
      const processNames = Object.keys(DarwinEditors);

      for (const processName of processNames) {
        if (output.includes(processName)) {
          return DarwinEditors[processName as keyof typeof DarwinEditors];
        }
      }
      ...
      for (const processPath of runningProcesses) {
        ...
        if (Win32Editors[processName as keyof typeof Win32Editors]) {
          return processPath;
        }
      }
      ...
      for (const processName in LinuxEditors) {
        if (output.includes(processName)) {
          return LinuxEditors[processName as keyof typeof LinuxEditors];
        }
      }
    }
  } catch (error) {
...
async function launchEditor(
  ...
  const args = lineNumber
    ? getEditorArgsForLineNumber(editor, fileName, lineNumber, columnNumber)
    : [fileName];

  if (editorChild && isTerminalEditor(editor)) {
  ...
  if (process.platform === 'win32') {
    ...
    // launch .exe files.
    editorChild = child_process.spawn('cmd.exe', ['/C', editor].concat(args), {
      stdio: 'inherit',
    });
  } else {
    editorChild = child_process.spawn(editor, args, { stdio: 'inherit' });
  }

  editorChild.on('exit', function (errorCode) {
    editorChild = null;
  ...
  editorChild.on('error', function (error) {
...
/**
 * Based on https://github.com/facebook/create-react-app/blob/main/packages/react-dev-utils/launchEditor.js
 */
function openEditorMiddleware() {
  return async (req: Request, res: Response) => {
    ...
    const lineNumber = line ? parseInt(line.toString()) : 1;
    const columnNumber = column ? parseInt(column.toString()) : 1;
    await launchEditor(fileName?.toString(), lineNumber, columnNumber);
