...
let environmentConfig = require(path.resolve(applicationFolder, './build/ima/config/environment.js'));
...
function appFactory() {
	delete require.cache[path.resolve(applicationFolder, './build/ima/app.server.js')];

	require(path.resolve(applicationFolder, './build/ima/app.server.js'))();
}

function languageLoader(language) {
	return require(path.resolve(applicationFolder, `./build/ima/locale/${language}.js`));
}
...
let clientApp = require('./lib/clientApp.js')(environment, logger, languageLoader, appFactory);
