...
> **TypeScript:** Again, instead of the constructor, we used `useState` hook from React and defined `Post` type
...
> **TypeScript: ** In our functional component we replaced the `_onChange()` method with `handleChange()` arrow function.
...
> **Typescript: ** In functional components, the `fire` method is provided
to us by the `useComponent` hook, which gives us access to the utility methods.
