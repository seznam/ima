...
```typescript
_validate(inputName: string, value: string) {
	const validStateKey = `${inputName}Valid`;
	const isValid = !!value;
...
(`app/model/post/PostEntity.js`) by adding the following line(s) to the
**constructor**:
...
> **TypeScript:** In Typescript also change the `PostData` type in `app/page/home/HomeController` to:
...
> **TypeScript:** Do not forget to update the `HomeControllerState` type.
...
```javascript
const pendingPost = this._postFactory.createEntity(
	Object.assign(
		{
			id: null,
			isSaved: false,
...
```jsx
const allPosts = this.props.pendingPosts.concat(this.props.posts);
...
React considers them to be the same post ([remember how we use the post ID to identify the post in the DOM?](./fetching-data#using-postservices-in-homecontroller)).
...
> **TypeScript:** As you can see, we have to completely refactor our types to match our input data.
...
```typescript
import { PostData, PostsApiData } from 'app/page/home/HomeController';
...
...update the body of the last `then` callback in the `onPostSubmitted()`
method:
...
> **TypeScript:** Note that in TypeScript we have to also change return type
for method `getPosts` in `app/model/post/PostService.ts` to `Promise<PostsApiData>`...

```tsx
getPosts(): Promise<PostsApiData> {
...
...and slightly adjust the rendering of the posts in `app/page/home/HomeView`:
...
(`app/component/post/Post`) at the end of the
...
detail in describing each component of IMA.js development stack or take a direct look at the [API](../api/@ima/core/classes/Bootstrap).
