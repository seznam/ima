...
In Typescript also change the `PostData` type in `app/page/home/HomeController` to:
```typescript
...
Note that in TypeScript we have to also slightly adjust the rendering of the posts in `app/page/home/HomeView.tsx`
