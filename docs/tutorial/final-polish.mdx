...
> **TypeScript: ** As you definitely noticed, again we got access to the functionality from the `AbstractComponent`
  by using another IMA hook, this time `useCssClasses`, that gives us direct access to the IMA CssClases.
...
> **TypeScript:** Note that in TypeScript we have to also slightly adjust the rendering of the posts in `app/page/home/HomeView.tsx`
